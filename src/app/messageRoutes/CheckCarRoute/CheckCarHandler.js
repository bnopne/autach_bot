...
class CheckCarHandler extends Handler {
  async handle(message) {
    this.eventBus.emit(
      EVENT_TYPES.USER_EXECUTES_COMMAND,
      new UserExecutesCommand(commands.checkcar, message.from.id, message.chat.id),
    );
    ...
    if (!repliedMessage) {
      await this.bot.sendMessage(
        message.chat.id,
        phrases.commands.checkcar.replyOnYourMessage(),
      );

      this.eventBus.emit(
        EVENT_TYPES.USER_FAILS_TO_EXECUTE_COMMAND,
        new UserFailsToExecuteCommand(commands.checkcar, message.from.id, message.chat.id),
      );
    ...
    if (repliedMessage.from.id !== message.from.id) {
      await this.bot.sendMessage(
        message.chat.id,
        phrases.commands.checkcar.notYourMessage(),
      );
    ...
    if (!(repliedMessage.photo)) {
      await this.bot.sendMessage(
        message.chat.id,
        phrases.commands.checkcar.cantSeePhoto(),
      );
    ...
    await this.bot.sendMessage(
      message.chat.id,
      phrases.commands.checkcar.done(),
      { reply_to_message_id: message.messageId },
    );
  }
}

module.exports = CheckCarHandler;
